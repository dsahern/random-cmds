...
__u32 csum_partial(const void *buff, int len, __u32 sum)
{
        return (__u32)add32_with_carry(do_csum(buff, len), sum);
}

static __u16 ipv4_csum(const void *buf, short nbytes)
{
	return csum_fold(csum_partial(buf, nbytes, 0));
}

static inline __u32 csum_tcpudp_nofold(__be32 saddr, __be32 daddr,
				       __u32 len, __u8 proto, __u32 sum)
{
	asm("  addl %1, %0\n"
	    "  adcl %2, %0\n"
	    "  adcl %3, %0\n"
...
static __u16 tcpudp_csum(__be32 sip, __be32 dip, unsigned char proto,
			 const unsigned char *buf, unsigned int len)
{
	__u32 csum;

	csum = csum_tcpudp_nofold(sip, dip, len, proto, 0);
	csum = csum_partial(buf, len, csum);
